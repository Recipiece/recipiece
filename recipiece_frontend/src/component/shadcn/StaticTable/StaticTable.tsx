    ...
    <thead>
      <tr className="m-0 border-t p-0 even:bg-muted">
        {allChildren.map((ch, index) => {
          return (
            <th key={index} className="border dark:border-muted px-4 py-2 text-left font-bold [&[align=center]]:text-center [&[align=right]]:text-right">
              {ch}
            </th>
          );
        })}
...
export const StaticTableRow: FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <tr className="m-0 border-t p-0">
      {allChildren.map((ch, index) => {
        return (
          <td key={index} className="border dark:border-muted px-4 py-2 text-left [&[align=center]]:text-center [&[align=right]]:text-right">
            {ch}
          </td>
        );
      })}
