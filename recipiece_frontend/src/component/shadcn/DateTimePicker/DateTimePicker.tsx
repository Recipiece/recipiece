...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    ...
    return format(displayValue, `${!hideTime ? "MMM" : "MMMM"} d, yyyy${!hideTime ? (use12HourFormat ? " hh:mm:ss a" : " HH:mm:ss") : ""}`);
      ...
      <PopoverContent className="w-auto p-2">
        ...
        <div className="relative overflow-hidden">
          <DayPicker
            ...
            {...props}
          />
          <div className={cn("absolute top-0 left-0 bottom-0 right-0", monthYearPicker ? "bg-popover" : "hidden")}></div>
          <MonthYearPicker
            value={month}
        ...
        <div className="flex flex-col gap-2">
          {!hideTime && <TimePicker timePicker={timePicker} value={date} onChange={setDate} use12HourFormat={use12HourFormat} min={minDate} max={maxDate} />}
          ...
          <div className="grid grid-cols-4">
            {years.map((year) => (
              <div key={year.value} ref={year.value === getYear(value) ? yearRef : undefined}>
                <Button disabled={year.disabled} variant={getYear(value) === year.value ? "default" : "ghost"} className="rounded-full" onClick={() => onYearChange(year)}>
                  {year.label}
                </Button>
              </div>
            ))}
