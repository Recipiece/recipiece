    ...
    <>
      {isValidMembershipId && (
        <Tooltip>
              ...
              <Avatar className={SIZE_CLASSNAME_MAP[size]}>
                <AvatarFallback className={cn(SIZE_CLASSNAME_MAP[size], "cursor-pointer bg-primary text-white")}>
                  <LoadingGroup isLoading={isLoadingMembership || isLoadingUser} variant="spinner" className="w-4 h-4">
                    {!isOwnedByUser && <>{owner.charAt(0).toUpperCase()}</>}
                    {isOwnedByUser && <Share2 className={ICON_SIZE_CLASSNAME_MAP[size]} />}
                  </LoadingGroup>
                </AvatarFallback>
              </Avatar>
            </div>
          ...
          <TooltipContent>
            <>{!!owner && !isOwnedByUser && <>{owner}</>}</>
            <>{isOwnedByUser && <>Shared with others</>}</>
          </TooltipContent>
        </Tooltip>
      )}
    </>
