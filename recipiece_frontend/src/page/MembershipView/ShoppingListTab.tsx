...
export const ShoppingListTab: FC<TabProps> = ({ userKitchenMembershipId }) => {
  ...
  const onDelete = useCallback(async (share: ListShoppingListSharesResponseSchema["data"][0]) => {
    try {
        ...
        {(sharesFromUser?.data ?? []).map((share) => {
          return (
            <div key={share.id} className="flex flex-row gap-2 items-center border-b pb-2">
              <a className="underline" href={`/shopping-list/view/${share.shopping_list.id}`}>{share.shopping_list.name}</a>
