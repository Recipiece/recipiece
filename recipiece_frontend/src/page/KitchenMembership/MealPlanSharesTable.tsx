...
export const MealPlanSharesTable: FC<{ readonly membership: UserKitchenMembershipSchema }> = ({ membership }) => {
  ...
  return (
    <LoadingGroup isLoading={isLoadingMealPlanShares} variant="spinner" className="h-10 w-10">
      ...
      {(mealPlanShares?.data?.length ?? 0) === 0 && <p className="text-center text-sm">You haven&apos;t shared any meal plans with this user.</p>}
    </LoadingGroup>
  );
};
